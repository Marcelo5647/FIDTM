...
from dataset_generation.generate_safety_grids import generate_real_safety_grid
...
def main():
    ...
    for filename in np.loadtxt(valid_list_txt_path, dtype=str):
        if filename.split('.')[1] == 'png':
            if severity == 0:
                ...
                img_paths.append(os.path.join(img_root, f'corrupted_images/gaussian_noise_{severity}', filename))
    img_paths.sort()
    ...
    for index, img_path in enumerate(img_paths):
        ...
        with torch.no_grad():
            ...
            depth_tensor = torch.tensor(np.asarray(depth)[np.newaxis, np.newaxis, np.newaxis, np.newaxis], dtype=torch.float).to(device)

            start = time.time()

            fidt_map = fidtm_model(img_tensor)
            pred_kpoint = LMDS_counting(fidt_map)
            safety_grid = generate_real_safety_grid(pred_kpoint, (3,3), img_info, min_safe_dist = 100, head_plane_height = 200, use_default_dist_map = True)
            end = time.time()

            # safety_grid = safety_grid.to('cpu').detach().numpy()[0,0,:,:]
    ...
    print(f'Average MSE Loss = {sum(mse_losses)/len(mse_losses)}')
    print(f'Mean Exec Time (ms) = {sum(exec_times)/len(exec_times)}')
